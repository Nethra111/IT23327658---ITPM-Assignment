...
function loadFunctionalCasesSync() {
  ...
  for (let r0 = meta.headerRowIndex0 + 1; r0 < rows.length; r0++) {
    ...
    if (!tcId) continue;
    if (!/^(Pos|Neg)_Fun_\d{4}$/.test(tcId)) continue;
...
const FUNC = loadFunctionalCasesSync();
...
test.describe("SwiftTranslator - Functional", () => {
  test.setTimeout(120000);

  test("Excel functional cases loaded", async () => {
    expect(FUNC.cases.length).toBeGreaterThan(0);
  });
  ...
  for (const c of FUNC.cases) {
    test(c.tcId, async ({ page }) => {
      ...
      const actual = await waitForOutput(page, prev, 25000);
      const status = isPass(actual, c.expected) ? "Pass" : "Fail";

      results.set(c.tcId, { actual: crop(actual), status });
      expect(canon(actual).length).toBeGreaterThan(0);
    });
  }

  test.afterAll(async () => {
    if (!FUNC.cases.length) return;
    ...
    for (const c of FUNC.cases) {
      ...
      row.getCell(FUNC.cols1.actual).value = res.actual;
      row.getCell(FUNC.cols1.status).value = res.status;
      row.commit();
    }
